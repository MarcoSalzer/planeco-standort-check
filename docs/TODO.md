...
**Nächster Schritt: Block (e)** (GitHub-Actions-Workflow), laut Marcos
Nachricht vom 17.08. der letzte Schritt vor "Phase 4 durch" - er nannte
Block (c) und (d) dabei nicht. Ob das eine bewusste Scope-Entscheidung ist
oder nur nicht erwähnt wurde, ist mit ihm NICHT bestätigt - als offene
Frage in den Block-(c)/(d)-Zeilen unten vermerkt, nicht eigenmächtig
übersprungen oder eigenmächtig doch noch gebaut. Ampel-Funktion existiert
